...
def Surf_Functionality():
    ...
    #These should be genertaed from suntimes, latitude and longitude for humboldt but it was giving me trouble so I just hard coded it. gangshit.
    ...
    for spots in surf_spots:
        ...
        #All the data frame/ pandas (pd) stuff is copy and pasted from surfline API example. not sure why it works  but it does.
        ...
        '''
        #Getting dawn and dusk times from the surfline forcast data was super buggy so I think I'll do it differently
        for sunlight_times in spot_forecasts.sunlightTimes:
            sun rise sunset is broken
            
            print("Dawn",sunlight_times.dawn.dt,"Dusk",sunlight_times.dusk.dt )
        '''
...
#straight copy and passte from google example there a file named token.json in with credentials to get info from my google calender.
...
# straight copy and pasted from calender API example i pass in start time and end time
...
def main():
  ...
  try:
    ...
    print("Getting the upcoming events for the next 2 days")
    ...
    # All surfable spots = looks at my
    All_Surfable_Spots = Surf_Functionality()
    
    '''
    for spots in All_Surfable_Spots:
        print("Spot Name:",spots["name"])
        if len(spots["data"]) != 0:
            surfable_hours_df = pd.DataFrame(spots["data"])
            print(surfable_hours_df[['timestamp_dt', 'surf_min', 'surf_max', 'speed', 'temperature']])
        else:
            print("There are no surfable hours")
    '''
